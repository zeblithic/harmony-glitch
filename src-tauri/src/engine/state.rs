...
pub struct SaveState {
    ...
    /// Imagination currency (earned from interactions and crafting).
    #[serde(default)]
    pub imagination: u64,
    ...
    pub fn tick(&mut self, dt: f64, input: &InputState, rng: &mut impl Rng) -> Option<RenderFrame> {
        ...
        if let Some(ref craft) = self.active_craft {
            if self.game_time >= craft.complete_at {
                ...
                // Award imagination for crafting
                self.imagination += 10;
                self.pickup_feedback.push(PickupFeedback {
                    id: self.next_feedback_id,
                    text: "+10 Imagination".to_string(),
                    success: true,
                    x: self.player.x,
                    y: self.player.y,
                    age_secs: 0.0,
                ...
                if let Some(nearest) = &nearest {
                    ...
                    match &result.interaction_type {
                        Some(interaction::InteractionType::Entity { entity_type }) => {
                            ...
                            // Award imagination for entity interaction
                            self.imagination += 5;
                            self.pickup_feedback.push(PickupFeedback {
                                id: self.next_feedback_id,
                                text: "+5 Imagination".to_string(),
                                success: true,
                                x: self.player.x,
                                y: self.player.y,
                                age_secs: 0.0,
